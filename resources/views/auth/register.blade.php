...
@section('content')
    <div class="w-full bg-white h-screen flex justify-center items-center">
        <form action="{{ route('registrasi.store') }}" wire:navigate method="POST" class="flex flex-col gap-2 border shadow-lg py-10 px-8 w-full mx-8 md:w-96 rounded-lg ">
            @csrf
            <span class="text-2xl text-center font-bold mb-4">Register</span>
            <label for="" class="">Username</label>
            <input type="name" name="username" id="" autofocus
                class="outline-none rounded-lg border border-blue-100 focus:ring-2 focus:ring-opacity-30 focus:ring-blue-300 py-2 px-4 "
                required autocomplete="off" placeholder="Input name">
            ...
            <div
                ...
                <input type="password" name="password" id=""
                    class="outline-none w-full py-2 px-4" required autocomplete="off"
