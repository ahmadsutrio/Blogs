    ...
    <form class="row g-3 mx-lg-3" method="post" action="{{ route('admin.store') }}" enctype="multipart/form-data">
        ...
        <div class="col-md-6">
            ...
            <select class="form-select @error('status') is-invalid @enderror" name="status" aria-label="Default select example" >
                ...
                <option value="private" {{ $data_blog['status'] == 'private' ? 'selected' : '' }}>Private</option>
                <option value="published" {{ $data_blog['status'] == 'published' ? 'selected' : '' }}>Published</option>
            </select>
             @error('status')
                <div class="invalid-feedback">
                    {{ $message }}
        ...
        <div class="col-md-6">
            ...
            <input class="form-control @error('foto') is-invalid @enderror" type="file" name="foto" id="formFile" value="{{ $data_blog['foto'] }}">
            <img src="{{ asset('/storage/'.$data_blog['foto']) }}" alt="" class="img-fluid" width="200px">
             @error('foto')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="col-md-6">
            ...
            <select class="form-select @error('id_kategori') is-invalid @enderror" name="id_kategori" aria-label="Default select example">
                ...
                @foreach ($getKategori as $item)
                    <option value="{{ $item->id }}"  {{ $data_blog['id_kategori'] == $item->id ? 'selected' : '' }}>{{ $item->kategori }}</option>
                @endforeach
            </select>
             @error('id_kategori')
                <div class="invalid-feedback">
        ...
        <div class="col-12">
            ...
            <textarea name="content" class="form-control @error('content') is-invalid @enderror" id="inputContent" cols="30" rows="10" >{{ $data_blog['content'] }}</textarea>
             @error('content')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
